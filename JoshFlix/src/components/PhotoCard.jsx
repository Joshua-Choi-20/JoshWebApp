...
function PhotoCard({ text, image, carouselButton }) {
  ...
  function onButtonClick({}) {
    ...
    return (
        <>
            <button onClick={onButtonClick} className="col-span-1 item flex justify-center items-center bg-cover"
...
export default function Gallery() {
    const photoData = [
        { text: "Kim's Convenience", image: '/src/assets/college_josh.PNG', backgroundIndex: 0},
        ...
        <>
            ...
            <div class="text-sm grid w-10/12 mx-10 container h-1/6 gap-3 grid-cols-3  md:grid-cols-6 mb-10 sm:grid-cols-6">
                {photoData.map((photo, index) => (
                    <PhotoCard key={index} text={photo.text} image={photo.image} backgroundIndex={photo.backgroundIndex} />
                ))}
            </div>
        </>
    )
